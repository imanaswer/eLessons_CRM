...
export default async function Integrations({ searchParams }: { searchParams: Promise<{ state?: string; map?: string }> }) {
  ...
  return <Page title="Integrations" sub={hq ? 'Every connection across all centres.' : 'Your centre\'s connections only.'}>
    {!ro && <section className="grid gap-4 md:grid-cols-2">
      <div className="panel p-4"><h2 className="font-medium">Facebook page (Meta Lead Ads)</h2><p className="mt-1 text-sm text-muted">Connect your page, sync its lead forms, and every lead from that page lands in your centre automatically. One G-TEC Meta app is used for all centres.</p>
      ...
      <ActionForm action={createConnectionAction} submit="Create" className="panel space-y-2 p-4"><h2 className="font-medium">Other connection</h2>
    ...
    {meta && <section className="panel p-4"><h2 className="font-medium">Map fields · {meta.name}</h2><p className="text-sm text-muted">Standard Meta questions (full_name, phone_number, email, city…) map automatically. Map custom questions here; the key is the question&apos;s field name in Meta.</p>
