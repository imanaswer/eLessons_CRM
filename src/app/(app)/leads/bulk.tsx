...
export function BulkBar({ children, perms, readOnly, owners, centres, lists, filters }: {
  ...
  perms: { assign: boolean; transfer: boolean; exp: boolean; pullBack?: boolean }; owners: Opt[]; centres: Opt[]; lists: Opt[]
}) {
  const [state, action, pending] = useActionState(bulkAction, null)
  const [op, setOp] = useState('')
  return (
    <form action={action} className="space-y-2" onSubmit={(e) => {
      if ((op === 'transfer' || op === 'pull_back') && !window.confirm('Transfer the selected leads? The current centre will lose access to them.')) e.preventDefault()
    }}>
      ...
      {!readOnly && (
        <div className="flex flex-wrap items-center gap-2 text-[13px]">
          <select name="op" value={op} onChange={(e) => setOp(e.target.value)} className="input w-auto" aria-label="Bulk action">
            ...
            {perms.pullBack && <option value="pull_back">Pull back to HQ pool</option>}
          ...
          {op === 'pull_back' && <input name="reason" className="input w-56" placeholder="Reason (required)" aria-label="Reason" required />}
