...
export default async function Opportunities({ searchParams }: { searchParams: Promise<{ view?: string; status?: string }> }) {
  ...
  const tabs = <div className="flex gap-1">{[['kanban', 'Kanban'], ['table', 'Table']].map(([k, l]) => <Link key={k} href={`?view=${k}&status=${status}`} className={`rounded-full border px-3 py-1 text-sm ${view === k ? 'border-ink bg-ink text-white' : 'border-line bg-surface text-muted'}`}>{l}</Link>)}
  ...
  const Move = ({ deal }: { deal: D }) => d.ro || deal.status !== 'open' ? null : (
    <ActionForm action={moveDealAction} submit="Move" className="mt-2 flex flex-wrap gap-1 text-xs">
      <input type="hidden" name="deal_id" value={deal.id} />
      <select name="stage_id" className="input h-7 w-auto text-xs" defaultValue={deal.stage_id} aria-label="Stage">{d.stages.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
      <input name="lost_reason" className="input h-7 w-28 text-xs" placeholder="Lost reason" aria-label="Lost reason" />
    </ActionForm>)
  return <Page title="Opportunities" sub="A deal belongs to one student. Won closes the lead as Enrolled; Lost needs a reason." action={tabs}>
    {view === 'kanban' ? <div className="flex gap-3 overflow-x-auto pb-2">{d.stages.filter((s) => status === 'open' ? !s.is_won && !s.is_lost : s.is_won || s.is_lost).map((s) => { const col = d.deals.filter((x) => x.stage_id === s.id); const total = col.reduce((n, x) => n + Number(x.amount), 0)
      return <section key={s.id} className="panel w-72 shrink-0"><h2 className="border-b border-line px-3 py-2 text-sm font-medium">{s.name} <span className="text-muted">{col.length} · {money(total, col[0]?.currency)}</span></h2>
        <ul className="divide-y divide-line">{col.map((x) => <li key={x.id} className="p-3 text-sm"><Link href={`/leads?view=all&lead=${x.lead_id}&tab=opportunities`} className="font-medium hover:underline">{x.lead ?? x.name}</Link>
          <p className="text-muted">{x.student ?? 'No student'} · {money(x.amount, x.currency)}{x.kind === 'renewal' && ' · renewal'}</p><p className="text-xs text-muted">{x.owner}{x.centre ? ` · ${x.centre}` : ''} · <LocalTime iso={x.last_moved_at.toISOString()} dateOnly /></p>{x.lost_reason && <p className="text-xs text-danger">{x.lost_reason}</p>}<Move deal={x} /></li>)}
          {col.length === 0 && <li className="p-3 text-sm text-muted">Empty</li>}</ul></section> })}</div>
    : <Table head={['Lead', 'Student', 'Stage', '#Amount', 'Owner', 'Centre', 'Expected close', 'Moved']} empty={d.deals.length ? undefined : 'No deals.'}>
