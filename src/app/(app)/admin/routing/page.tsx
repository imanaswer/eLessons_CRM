...
export default async function Routing() {
  ...
  if (!d.hq) return <Page title="Routing"><p className="text-sm text-muted">You don&apos;t have permission to access this. Routing rules are managed by HQ.</p></Page>
  ...
  return <Page title="Routing" sub="Leads from a centre's own sources need no rule. HQ sources are routed by the first matching rule in priority order, else the fallback owner, else they wait in the HQ pool.">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      {!d.ro && <ActionForm action={routingRuleAction} submit="Add rule" className="panel space-y-2 p-4"><h2 className="font-medium">New rule</h2>
      ...
      <div className="space-y-4">
        <ActionForm action={routePreviewAction} submit="Where would it go?" className="panel space-y-2 p-4"><h2 className="font-medium">Diagnose</h2><p className="text-sm text-muted">Describe a lead; see which rule catches it. Nothing is saved.</p>
