...
export default async function Engagements({ searchParams }: { searchParams: Promise<{ lens?: string; days?: string }> }) {
  ...
  return <Page title="Engagements" sub={`Last ${days} days · times shown in IST`} action={<div className="flex flex-wrap gap-1">{Object.keys(LENSES).map((k) => <Link key={k} href={`?lens=${k}&days=${days}`} className={`rounded-full border px-3 py-1 text-sm capitalize ${lens === k ? 'border-ink bg-ink text-white' : 'border-line bg-surface text-muted'}`}>{k}</Link>)}<Link href={`?lens=${lens}&days=${days === 7 ? 30 : 7}`} className="btn btn-quiet h-8">{days === 7 ? '30 days' : '7 days'}</Link></div>}>
