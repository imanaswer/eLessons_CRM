...
export default async function Conversations({ searchParams }: { searchParams: Promise<{ tab?: string; inbox?: string; id?: string }> }) {
  ...
  return <Page title="Conversations" action={<div className="flex gap-1">{[['mine', 'Lead Inbox'], ['team', 'Team Inbox']].map(([k, l]) => <Link key={k} href={`?inbox=${k}&tab=${tab}`} className={`rounded-full border px-3 py-1 text-sm ${inbox === k ? 'border-ink bg-ink text-white' : 'border-line bg-surface text-muted'}`}>{l}</Link>)}</div>}>
    <nav className="-mx-4 flex gap-1 overflow-x-auto px-4 text-sm">{Object.entries(TABS).map(([k, l]) => <Link key={k} href={`?inbox=${inbox}&tab=${k}`} className={`whitespace-nowrap rounded-full border px-3 py-1 ${tab === k ? 'border-ink bg-ink text-white' : 'border-line bg-surface text-muted'}`}>{l}</Link>)}</nav>
    <div className="grid gap-4 lg:grid-cols-[22rem_1fr]">
      <ul className="panel divide-y divide-line">{rows.map((c) => { const left = c.session_expires_at ? Math.max(0, Math.round((+c.session_expires_at - Date.now()) / 3600e3)) : null
        return <li key={c.id}><Link href={`?inbox=${inbox}&tab=${tab}&id=${c.id}`} className={`block p-3 text-sm hover:bg-canvas ${sp.id === c.id ? 'bg-blue-50' : ''}`}>
          <div className="flex items-center gap-2"><span className="truncate font-medium">{c.starred ? '★ ' : ''}{c.lead ?? c.wa_id}</span>{c.unread > 0 && <span className="rounded-full bg-brand px-1.5 text-xs text-white">{c.unread}</span>}<span className="ml-auto text-xs text-muted">{c.last_message_at && <LocalTime iso={c.last_message_at.toISOString()} />}</span></div>
        ...
        {rows.length === 0 && <li className="p-3 text-sm text-muted">No conversations in this tab.</li>}</ul>
      {sp.id ? <Thread id={sp.id} /> : <div className="panel grid place-items-center p-8 text-sm text-muted">Pick a conversation.</div>}
    </div>
  </Page>
}
